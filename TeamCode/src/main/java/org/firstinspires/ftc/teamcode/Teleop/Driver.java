...
public class Driver extends OpMode {
    ...
    private void armController(double targetPos) {
        armPIDConfig.target = (int) mapFloatToAngle(gamepad2.right_stick_y);
        ...
        double pid = armPID_Controller.calculate(armPos, armPIDConfig.target);
        double ff = Math.cos(Math.toRadians(armPIDConfig.target / TICKSINDEGREES)) * armPIDConfig.f;

        double power1 = pid + ff;
        double power2 = pid + ff;
